...
def build_models():
    ...
    if LGBM_AVAILABLE:
        ...
        models["lightgbm"] = TransformedTargetRegressor(
            regressor=lgbm_pipe,
            func=np.log1p,
            inverse_func=np.expm1,
        )
    if LGBM_AVAILABLE and XGBOOST_AVAILABLE:
      stack_pipe = Pipeline([
      ("imputer", SimpleImputer(strategy="median")),
    ...
    models["stacking"] = TransformedTargetRegressor(
...
def run_single_holdout(df, test_month=None, use_all_history=False):
    ...
    if best_model_name in {"random_forest", "xgboost", "lightgbm", "stacking"}:
